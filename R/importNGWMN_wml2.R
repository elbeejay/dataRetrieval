...
importWaterML2 <- function(input, asDateTime = FALSE, tz = "UTC") {
  ...
  if (asDateTime) {
    timeDF$dateTime <- lubridate::parse_date_time(timeDF$dateTime, c(
      "%Y", "%Y-%m-%d", "%Y-%m-%dT%H:%M", "%Y-%m-%dT%H:%M:%S",
  ...
  tvpQuals <- xml_text(xml_find_all(
    returnedDoc,
  ...
  df_use <- df_vars[sapply(df_vars, function(x) {
  ...
  mdAttribs <- list(
    defaultQualifier = defaultQuals, defaultUOM = defaultUOM,
    gmlID = gmlID
  ) # all attributes must have names
  mdAttribs_use <- mdAttribs[sapply(mdAttribs, function(x) {
    length(x) > 0
  })]
